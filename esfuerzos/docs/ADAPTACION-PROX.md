...
1. Leer todos los campos acumulados del contexto de conversación
2. Hashear el `client_phone` (SHA-256) → `reporter_wa_hash`
3. Escribir el registro en `reports`
4. Vincular las fotos en `photos`
5. Limpiar el contexto de conversación
...
| # | Paso | Motivo |
|---|------|--------|
| 1 | Limpieza | Desbloquea que la app arranque |
| 2 | `app/models/reporte.py` | Base de datos necesaria antes del bot |
| 3 | Seeder | Sin esto el bot es mudo |
| 4 | `flow_seeder.py` reescrito | Flujos conversacionales |
| 5 | Lógica de fotos múltiples en Orchestrator | Agrupación de mensajes en burst |
| 6 | `app/core/intake.py` | Persistencia al finalizar el flujo |
| 7 | `send_match_notification` en `waha.py` | Notificación de salida |
