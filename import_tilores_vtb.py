...
2. Run from the repo root (or inside the container):
       ...
       python import_tilores_vtb.py --file /path/to/tilores_export.jsonl

   Or, if Tilores gave you a signed download URL:

       python import_tilores_vtb.py --url "https://signed-url..." --file tilores_export.jsonl
...
- source_url dedup key: tilores_vtb:{cedula} > tilores_vtb:{entity_id} > tilores_vtb:{hash}
...
def normalize(record: dict) -> dict | None:
    ...
    contacts   = _coerce_str(_pick(record, FIELD_MAP["contacts"]))
    ...
    if contacts:
        marks_parts.append(f"Contactos: {contacts}")
